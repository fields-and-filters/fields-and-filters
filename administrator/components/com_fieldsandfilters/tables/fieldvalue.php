...
class FieldsandfiltersTablefieldvalue extends JTable
{
	...
	{
		parent::__construct( '#__fieldsandfilters_field_values', 'field_value_id', $db );
	}
	...
	public function check()
	{
		...
		if( trim( $this->field_value_alias ) == '' )
		{
			$this->field_value_alias = $this->field_value;
		}
	       
		$this->field_value_alias = JApplication::stringURLSafe( $this->field_value_alias );
	       
		if( trim( str_replace( '-', '', $this->field_value_alias ) ) == '' )
		{
	...
	public function store( $updateNulls = false )
	{
		...
		if( $table->load( array( 'field_value_alias' => $this->field_value_alias, 'field_id' => $this->field_id ) ) && ( $table->field_value_id != $this->field_value_id || $this->field_value_id == 0 ) )
		{
			$this->setError( JText::sprintf( 'COM_FIELDSANDFILTERS_DATABASE_ERROR_FIELD_VALUE_ALIAS_EXISTS', $this->field_value_alias ) );
			return false;
		}
